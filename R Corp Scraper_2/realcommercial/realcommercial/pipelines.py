...
class RealcommercialPipeline(object):
...
class MyPipeline(object):
    ...
    def spider_opened(self, spider):
        ...
        try:
            self.writer = pd.ExcelWriter('C:\\Users\\Ravi Shankar\\Documents\\Upwork\\R Corp Scraper_2\\commercial_real_estate\\real_estate_data.xlsx')
            self.df_data = pd.read_excel('C:\\Users\\Ravi Shankar\\Documents\\Upwork\\R Corp Scraper_2\\commercial_real_estate\\real_estate_data.xlsx','Sheet1')
        except:
            self.writer = pd.ExcelWriter('C:\\Users\\Ravi Shankar\\Documents\\Upwork\\R Corp Scraper_2\\commercial_real_estate\\real_estate_data.xlsx', engine='xlsxwriter')
    ...
    def process_item(self, item, spider):
        if len(item.keys())>1:        
            if self.df_data.query('Address == "' +item['Address'] + '"').shape[0] == 0:
                print("RealCommercial spider scraping url :: " + item['url'])
