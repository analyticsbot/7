...
class RealCommercial(CrawlSpider):        
    ...
    def parse_file_page(self, response):
        ...
        item['Categories'] = str(sel.xpath('//*[@class="propType last"]/text()'). extract()[0]).strip()
        ...
        item['CaptionType'] = 'NA'
        try:
            item['LastUpdated'] = str(parser.parse(listings[listings.index('Last Updated')+1]).date())
        except:
            item['LastUpdated'] = 'NA'
