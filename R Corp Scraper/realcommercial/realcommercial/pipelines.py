...
class RealcommercialPipeline(object):
    def process_item(self, item, spider):
        if 'Address' not in item.keys():
            print 'AddressAddressAddressAddressAddressAddress'
        return item
...
class MyPipeline(object):
    ...
    def spider_opened(self, spider):
        ...
        try:
            self.book = load_workbook('real_commercial_data.xlsx')
            self.writer = pandas.ExcelWriter('real_commercial_data.xlsx') 
            self.writer.book = self.book
            self.writer.sheets = dict((ws.title, ws) for ws in self.book.worksheets)
        except:
            self.writer = pd.ExcelWriter('real_commercial_data.xlsx', engine='xlsxwriter')
