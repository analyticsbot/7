...
try:
    book = load_workbook('commercial_real_estate_data2.xlsx')
    writer = pandas.ExcelWriter('commercial_real_estate_data2.xlsx') 
    writer.book = book
    writer.sheets = dict((ws.title, ws) for ws in book.worksheets)
except:
    writer = pd.ExcelWriter('commercial_real_estate_data2.xlsx', engine='xlsxwriter')
    cols_data = __hdr
